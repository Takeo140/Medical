...
theorem all_protocols_safer_than_ips :
  ...
  rcases hp with rfl | rfl | rfl | rfl | rfl <;> norm_num
    [protocol_dna_to_neuron, protocol_dna_to_cardio,
     protocol_dna_to_hepato, protocol_dna_to_beta, protocol_dna_to_osteoblast]
...
-- 1. 最適化・拡張した神経細胞(Neuron)を作るDNAプロトコルを検索
#eval find_protocol_by_target SomaticType.Neuron

-- 2. 設計遺伝子カセットごとの最適化スコア算出
#eval protocol_db.map (fun p => (p.name, safety_efficiency_score p))

-- 3. DNA 1,000,000 ユニットから生成される推定体細胞数（神経細胞のyieldを検証）
