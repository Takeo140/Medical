...
inductive TranscriptionFactor : Type
  ...
  | Gata4 | Mef2c | Tbx5 | Hand2 | Gata6
  | Hnf4a | Foxa1 | Foxa2 | Foxa3 | Cdx2
  | Pdx1 | Ngn3 | MafA
  | Oct4 | Sox2 | Klf4 | cMyc
  deriving Repr, DecidableEq
...
/-- Miura 2017: 線維芽細胞 → 大腸壁細胞 (腸管前駆細胞経由の直接変換転写因子) -/
def protocol_fibro_to_colon : DirectReprogrammingProtocol :=
  { name          := "Miura2017_iCol"
    source        := SomaticType.Fibroblast
    target        := SomaticType.ColonEpithelial
    factors       := [.Hnf4a, .Foxa3, .Gata6, .Cdx2]
    efficiency    := 0.10
    duration_days := 14
    tumor_risk    := 0.02 }
...
theorem all_protocols_safer_than_ips :
  ...
  rcases hp with rfl | rfl | rfl | rfl | rfl <;> norm_num
    [protocol_fibro_to_neuron, protocol_fibro_to_cardio,
     protocol_fibro_to_hepato, protocol_exo_to_beta, protocol_fibro_to_colon]
...
-- 皮膚（線維芽細胞）から大腸壁細胞へのプロトコル検索
#eval find_protocol SomaticType.Fibroblast SomaticType.ColonEpithelial

-- 全プロトコルのスコア（新規プロトコルを含む）
